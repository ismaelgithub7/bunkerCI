...
class StockProductionLot(models.Model):
    ...
    def find_or_create_lot(self, lot_name, product_id, create):
        ...
        if not lot_id:
            ...
            sql = "select * from stock_production_lot where name = '{}' and product_id = {}".format(lot_name.upper(), product_id.id)
            self._cr.execute(sql)
            if self._cr.fetchall():
                raise ValidationError ('La pda no permite crear números de serie duplicados para el mismo artículo. Deberás anular los números de serie')
            val = {'name': lot_name.upper(),
