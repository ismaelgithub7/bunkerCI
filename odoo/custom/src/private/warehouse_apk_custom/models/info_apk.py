...
class InfoApk(models.AbstractModel):
    ...
    def get_apk_product(self, code):
        ...
        if len(product_id)>1:
            raise ValidationError ('Se han encontrado varios productos para este código {}'.format(code))
        return product_id
    ...
    def get_apk_lot(self, code, product_id):
        ...
        if product_id:
            if code == product_id.barcode or code == product_id.default_code or code == product_id.wh_code:
                raise ValidationError ('No puedes crear el lote {} para el producto {}. El lote esta en los campos del producto'.format(code, product_id.default_code))
            domain += [('product_id', '=', product_id.id)]
        lot_id = self.env['stock.production.lot'].search(domain)
        if len(lot_id) > 1:
            raise ValidationError ('Se han encontrado varios lotes para este código {} y este productp: '.format(code, product_id.display_name))
        return lot_id
    ...
    def get_apk_location(self, code):
        ...
        if len(location_id) > 1:
            raise ValidationError('Se han encontrado varias ubicaciones para este código {}'.format(code))
        return location_id
    ...
    def m2o_dict(self, field):
        if field:
            return {'id': field.id, 'name': field.apk_name, 'wh_code': field.wh_code}
        else:
            return {'id': False}
    ...
    def get_apk_object(self, values):
        _logger.info ("APK: INFO_APK. ENtrada get_apk_object para {}".format(self._name, values))
        model = values.get('model', False)
    ...
    def get_model_object(self, values={}):
        _logger.info("APK: INFO_APK. Entrada get_model_object para {} {}".format(self._name, values))
    ...
    def change_field_value(self, values):
        ...
        try:
            ...
            raise ValidationError('error al escribir {} en el campo {}. Model {}. Id {}'.format(model, id, field, value))
        return
