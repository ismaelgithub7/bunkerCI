...
class StockMoveLine(models.Model):
    ...
    def return_fields(self, mode='tree'):
        return ['id', 'product_uom_qty', 'qty_done', 'location_id', 'location_dest_id', 'lot_id', 'lot_name',
    ...
    def update_move_location(self, values):
        ...
        if not sml_ids and new_loc_id:
            ...
            sml_ids = move_id.move_line_ids.filtered(lambda x: x[field_location]['id'] == old_loc_id and x.qty_done == 0)
        elif sml_ids:
    ...
    def assign_location_to_moves(self, values):
        ...
        default_location = move_id.default_location
        if move_id:
            ##Escribo en todos los move_lines la ubicación que acabo de leer, siempre y cuando sean distinta y no estén hecho
            sml_ids = move_id.move_line_ids.filtered(lambda x: x[default_location].barcode != barcode and x.qty_done == 0)
        if not sml_ids:
            move_id.active_location_id = location_id
